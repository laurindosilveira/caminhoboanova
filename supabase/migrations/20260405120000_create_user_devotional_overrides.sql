CREATE TABLE IF NOT EXISTS public.user_devotional_overrides (
  ...
  granted_by uuid NULL REFERENCES public.profiles(user_id) ON DELETE SET NULL,
  ...
  CONSTRAINT user_devotional_overrides_user_devotional_key UNIQUE (user_id, devotional_id)
);
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'user_devotional_overrides_custom_points_nonnegative'
  ) THEN
    ALTER TABLE public.user_devotional_overrides
      ADD CONSTRAINT user_devotional_overrides_custom_points_nonnegative
      CHECK (custom_points IS NULL OR custom_points >= 0);
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_user_devotional_overrides_user_id
  ON public.user_devotional_overrides (user_id);

CREATE INDEX IF NOT EXISTS idx_user_devotional_overrides_devotional_id
  ON public.user_devotional_overrides (devotional_id);

DROP TRIGGER IF EXISTS update_user_devotional_overrides_updated_at ON public.user_devotional_overrides;
CREATE TRIGGER update_user_devotional_overrides_updated_at
BEFORE UPDATE ON public.user_devotional_overrides
FOR EACH ROW
EXECUTE FUNCTION public.update_updated_at_column();

ALTER TABLE public.user_devotional_overrides ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Users can view own devotional overrides" ON public.user_devotional_overrides;
CREATE POLICY "Users can view own devotional overrides"
...
CREATE POLICY "Admins can manage devotional overrides in area"
  ON public.user_devotional_overrides
  FOR ALL
  TO authenticated
  USING (
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE p.user_id = user_devotional_overrides.user_id
...
DROP POLICY IF EXISTS "Leaders can manage devotional overrides in area" ON public.user_devotional_overrides;
CREATE POLICY "Leaders can manage devotional overrides in area"
  ON public.user_devotional_overrides
  FOR ALL
  TO authenticated
  USING (
...
ALTER TABLE public.devotional_progress
  ...
  ADD COLUMN IF NOT EXISTS override_release_id uuid NULL REFERENCES public.user_devotional_overrides(id) ON DELETE SET NULL;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'devotional_progress_awarded_points_nonnegative'
  ) THEN
    ALTER TABLE public.devotional_progress
      ADD CONSTRAINT devotional_progress_awarded_points_nonnegative
